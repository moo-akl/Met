...
- `app/(tabs)/connections.tsx` — Connections tab: searchable chat-list of all `status === "connected"` encounters sorted by last activity. Each row shows avatar, name, timestamp, and a context-aware preview (real reply text · `You: <sent>` · last meeting location · `Met N times` fallback — never a "tap to message" call-to-action because messaging is intentionally secondary here). Top search bar filters by name with a clear button and dedicated "no matches" empty state.
- `app/connection/[id].tsx` — Connection profile + conversation. Profile/info (large avatar, name, "Met N times", bio, meeting-spot map card, social chips) is the **primary** content shown at the top of the screen by default — no toggle. Below it sits a "CONVERSATION" section divider, the chat thread, and a sticky composer (upgrade card for Free, per-day quota counter for Plus/Pro, "wait for reply" hint while a message is pending). Header subtitle is the static "Met N times". After connection, all message UX lives here — the encounter detail screen redirects via `router.replace` so a connected encounter is never shown twice.
