...
- **Android `versionCode`**: Must be incremented by 1 for *every* new Android build uploaded to Play Store, regardless of `expo.version`. Current: 209. **Always bump this before triggering a new build.**
- **iOS `buildNumber`**: Must be incremented by 1 for *every* new iOS build uploaded to TestFlight/App Store. Current: 209. **Always bump this before triggering a new build.**
