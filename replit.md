...
- `app/(tabs)/index.tsx` — Home: animated pulse beacon, **animated count-up** "X people within Nm" hero (via `hooks/useCountUp.ts` rAF easing), red **LIVE pulse dot** (`#EF4444` Animated opacity loop) next to "BEACON ACTIVE", **vibe pill** below the hero (Quiet zone / A few souls nearby / Lively here based on count), **rotating activity ticker** (4s interval, fade transition through `recent.slice(0,5)`), animated stat cards, soft radial-glow `LinearGradient` behind the active beacon, plus the **"This week"** recap card showing `newPeople` (firstSeen ≤7d) and `repeats` (encounterCount > 1 AND lastSeen ≤7d) with a "remember the human, not the follower count" tagline.
- `app/(tabs)/recent.tsx` — Recent encounters list with ScanFab → `/scan`
- `app/(tabs)/profile.tsx` — Editable own profile, MyQrSheet (grid icon), SettingsSheet (gear icon). Profile model also stores `photoVerifiedAt` so the verified-photo row can show "Last verified <date>", and `extraPhotos: string[]` for the tier-gated gallery. Bio TextInput placeholder is `"Bio"` (was "One sentence about you."). A **Photos** section below the avatar shows the main photo with a `MAIN` badge, each extra photo (tap → ActionSheet with Set as main / Remove), and an Add tile. On Free tier the Add tile renders a lock icon + `PLUS` pill and routes to `/paywall`; on Plus/Pro it pops the picker → `PhotoVerifier` → pushes the verified URI into `extraPhotos` (capped by tier). Promote swaps the promoted extra into the main slot, demotes the previous main back into `extraPhotos[i]`, and stamps `photoVerifiedAt = Date.now()` since the promoted photo was already verified at upload.
...
- `components/SettingsSheet.tsx` — Sectioned settings sheet with sub-views (`menu` | `blocked` | `notifications` | `about`):
  ...
  - **ACCOUNT** — **Verified photo** (tap opens `PhotoVerifier` modal that re-runs face + content checks against the existing `photoUri`; on success calls `markPhotoVerified` to stamp `photoVerifiedAt`), Blocked people, **About Met** sub-view (logo + version from `Constants.expoConfig?.version` + Privacy/Terms/Contact/Rate links via `expo-linking`), **Sign out** (inline placeholder card; real sign-out arrives with accounts), **Reset profile**, **Delete account** (destructive light-red row with trash icon → inline confirm card → `resetAll()` wipes profile/encounters/permissions/preferences; `ProfileGate` then redirects to onboarding).
...
- `lib/photoVerify.ts` — Two-stage photo verification API. Exports `runFaceCheck(uri)` and `runContentCheck(uri)` (each ~700ms, always succeed in this prototype) plus the atomic `verifyPhoto(uri)` and shared `PHOTO_VERIFY_FAIL_REASONS` copy. Real implementation would swap in `google_mlkit_face_detection`-style detection (reject 0 or >1 faces) + a content-safety classifier.
- `components/PhotoVerifier.tsx` — Full-screen modal that drives the two-stage UX (face → content) with a per-run monotonic `runIdRef` token so cancel/restart/close can never fire a stale `onVerified`. Auto-dismisses ~350ms after success; on failure shows reason + Retry (disabled while running) / Cancel. Wired into onboarding photo step (gates Continue), profile main + extra photo pickers, and Settings → Verified photo Re-verify.
- `lib/storage.ts` — Defines `MAX_EXTRA_PHOTOS_BY_TIER = { free: 0, plus: 2, pro: 5 }`. Total visible photos per tier = 1 main + extras (so 1 / 3 / 6).
- `hooks/useCountUp.ts` — rAF-based easing hook for animated number transitions. Used by the Home hero count and stat cards.
