...
## Production API URL + BLE background (Build #45 / versionCode 23)
- **Production API URL fix**: All four `eas.json` build profiles (`development`, `preview`, `preview:device`, `production`) updated from the unreliable Replit dev workspace URL (`*.kirk.replit.dev`, sleeps when workflow stops) to the deployment URL `https://metapp.replit.app`. This fixes the recurring "couldn't reach the server" errors on visibility toggle and the silent profile-upload failures during onboarding — both were caused by the dev URL being unreachable when the workspace was idle. Requires the API server to remain published.
- **BLE background encounters (Option A, BLE-only)**: encounters now keep firing while the app is backgrounded.
  - **iOS**: added `UIBackgroundModes: ["bluetooth-central", "bluetooth-peripheral"]` to `app.json` infoPlist, updated `NSBluetoothAlwaysUsageDescription` to mention background. `MetBleModule.swift` peripheral manager now constructs with `CBPeripheralManagerOptionRestoreIdentifierKey: "MetBlePeripheralRestore"` so iOS can resurrect it after termination. `lib/ble/scanner.ts` passes `restoreStateIdentifier: "MetBleCentralRestore"` to `react-native-ble-plx`'s BleManager. **Caveat (iOS limitation)**: when an iOS app advertises in the background, the OS strips `localName` and `serviceData` — only the service UUID survives. So iOS-bg ↔ iOS-bg detects a peer's presence (service UUID match) but cannot extract the identity hash payload from another backgrounded iOS device, meaning that specific pair won't generate an encounter until at least one of them comes to foreground. iOS-bg ↔ Android (any state) and Android-bg ↔ Android-bg still work fully because Android does not strip the service-data payload.
  - **Android**: new `MetBleService.kt` foreground service (notification "Met — Detecting nearby people", channel `met_ble_proximity`, low importance, foregroundServiceType `connectedDevice`). `MetBleModule.kt` starts the service the moment any BLE activity begins (legacy GATT advertise, iBeacon advertise, or scan) and stops it once both `fgAdvertising` and `fgScanning` refcount booleans go false. Service uses `START_NOT_STICKY` so the system can't resurrect the notification without actual BLE work. Permissions added: `FOREGROUND_SERVICE`, `FOREGROUND_SERVICE_CONNECTED_DEVICE`, `POST_NOTIFICATIONS` (in `app.json` and the `with-met-ble.js` plugin). Service registered in the module's `AndroidManifest.xml` with `android:exported="false"` and `foregroundServiceType="connectedDevice"`.

## GATT-on-detection (Build #46 / versionCode 24)
- **Reason**: iOS ↔ iOS background BLE has a known Apple limitation — when both iPhones are backgrounded, the OS strips `localName` and `serviceData` from outgoing advertisements; only the service UUID survives. Build #45's UIBackgroundModes fix kept the radio alive but couldn't fix this payload-stripping. Build #46 closes the gap by having the scanner briefly connect to peers whose advertisement carries our service UUID but no extractable identity hash.
- **Wire**: new read-only characteristic `MET_HASH_CHARACTERISTIC_UUID` = `4d455400-7770-4ac2-9b3d-000000000002` on the existing Met service. Returns the 8-byte identity hash bytes. Defined in `lib/ble/uuids.ts`.
- **iOS** (`MetBleModule.swift`): `doStart` now also calls `ensureGattService` to add a `CBMutableService` containing one read-only `CBMutableCharacteristic` to the `CBPeripheralManager`. The peripheral delegate's `didReceiveRead` responds with the cached 8 bytes (or the appropriate offset slice). Service is recreated whenever the hash changes (different uid) and torn down on `stopAdvertising`. Also handles `willRestoreState` so iOS resurrection doesn't crash.
- **Android** (`MetBleModule.kt`): switched `setConnectable(false)` → `setConnectable(true)` on the legacy GATT advertiser so iOS scanners can initiate connections. New `ensureGattServer` opens a `BluetoothGattServer` with the same service+characteristic and responds in `onCharacteristicReadRequest` with the 8 bytes. Server is torn down on `stopGattAdvertising`. Android peers normally still match via `serviceData` and never connect — this server is symmetric/defense-in-depth.
- **JS scanner** (`lib/ble/scanner.ts`):
  - When `extractHash` returns null on a Met-service-UUID advertisement, the device gets queued via `maybeQueueGattRead(deviceId, rssi)`.
  - Per-device state machine: `idle` → `pending` → `connecting` → `cooldown`. `MAX_PARALLEL_CONNECTS = 2` to keep radio congestion low. `GATT_CONNECT_TIMEOUT_MS = 6_000`, `GATT_OVERALL_TIMEOUT_MS = 8_000` (forces a `cancelDeviceConnection` if the read hangs).
