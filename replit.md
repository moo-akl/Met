...
- **Modular headers fix (RNFB v24 + Expo SDK 54 + Xcode 26)**: `useFrameworks: "static"` (required by Firebase iOS SDK) makes RNFBApp emit "non-modular include in framework module" warnings on its `<React/RCTBridgeModule.h>` import. The custom plugin `plugins/with-modular-headers.js` (registered in `app.json` plugins) injects exactly two things into the prebuilt Podfile: (1) `$RNFirebaseAsStaticFramework = true` at the very top, which is the official RNFB recommendation that opts every RNFB pod into static-framework packaging with its own module map (instead of forcing it globally), and (2) `CLANG_ALLOW_NON_MODULAR_INCLUDES_IN_FRAMEWORK_MODULES = YES` on every pod target inside the `post_install` hook. Plugin requires `@expo/config-plugins` as a direct devDep (pnpm doesn't hoist it for EAS to find). **Do NOT add `use_modular_headers!` or `DEFINES_MODULE = YES`** — earlier versions of this plugin did, which forced every `<React/RCTBridgeModule.h>` import into a *module* import. Module imports cross types/protocols across the framework boundary but **strip preprocessor macros**, causing `RCT_EXTERN`, `RCT_EXPORT_METHOD`, and `RCT_CONCAT` to compile as unknown identifiers ("unknown type name 'RCT_EXTERN'", "duplicate declaration of method 'RCT_CONCAT'", "expected method body"). The companion plugin `plugins/with-rnfb-firestore-header-fix.js` prepends `#import <RNFBApp/RNFBAppModule.h>` above each `#import <React/RCTBridgeModule.h>` in the RNFB submodule headers (skips `app` to avoid circular imports) — harmless without `use_modular_headers!` since both imports stay textual. To debug Podfile changes locally without burning an EAS build, copy `artifacts/met` to `/tmp/met-prebuild`, symlink `node_modules` from the workspace, and run `npx expo prebuild --platform ios --no-install` — the generated `ios/Podfile` shows exactly what EAS will see. Always `rm -rf ios` before re-prebuilding because `--no-install` reuses the existing folder.
