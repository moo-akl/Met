...
// ─── Crash diagnostics (build 213) ──────────────────────────────────────────
// Show a native Alert with the JS error message BEFORE calling the original
// crash handler. Alert.alert() talks directly to native UI (same as RedBox)
// so it works even when the JS thread is in a fatal-error state. The app
// only crashes after the user taps OK — giving us the message without Xcode.
type _EUGlobal = {
  ErrorUtils?: {
    getGlobalHandler: () => ((e: Error, isFatal: boolean) => void) | null;
    setGlobalHandler: (h: (e: Error, isFatal: boolean) => void) => void;
  };
};
const _origCrashHandler =
  (globalThis as unknown as _EUGlobal).ErrorUtils?.getGlobalHandler?.() ?? null;

(globalThis as unknown as _EUGlobal).ErrorUtils?.setGlobalHandler?.(
  (error: Error, isFatal: boolean) => {
    const msg =
      `fatal=${String(isFatal)}\n` +
      `${error?.message ?? "(no message)"}\n\n` +
      `${(error?.stack ?? "(no stack)").slice(0, 800)}`;
    console.error("[Met crash 213]", msg);

    // Guard: only call original handler once.
    let crashed = false;
    const doCrash = () => {
      if (crashed) return;
      crashed = true;
      if (_origCrashHandler) _origCrashHandler(error, isFatal);
    };

    // Show Alert — user reads the message and taps OK to let the crash proceed.
    // Fallback timeout (30 s) in case Alert itself fails to render.
    Alert.alert(
      "\u26a0\ufe0f JS Crash (tap OK to continue)",
      msg.slice(0, 600),
      [{ text: "OK", onPress: doCrash }],
      { cancelable: false },
    );
    setTimeout(doCrash, 30000);
  },
);
// ────────────────────────────────────────────────────────────────────────────
