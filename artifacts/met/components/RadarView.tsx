    ...
    <View style={{ width: size, height: size }}>
      <Svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        <Line
          ...
          x2={cx}
          y2={cy + maxR}
          stroke={hexToRgba(primaryColor, 0.06)}
          strokeWidth="1"
        />
        <Line
          x1={cx - maxR}
        ...
        <Line
          ...
          x2={sweepEnd.x}
          y2={sweepEnd.y}
          stroke={hexToRgba(primaryColor, 0.75)}
          strokeWidth="1.5"
          strokeLinecap="round"
        />
          ...
          return (
            <G key={i}>
              <Circle
                ...
                cy={p.y}
                r={glowR}
                fill={hexToRgba(primaryColor, bright * 0.15)}
              />
              <Circle
                cx={p.x}
                cy={p.y}
                r="9"
                fill={hexToRgba(primaryColor, 0.08 + bright * 0.12)}
                stroke={hexToRgba(primaryColor, alpha)}
                strokeWidth="1"
              />
              <SvgText
                x={p.x}
                ...
                fontSize="7"
                fontWeight="700"
                fill={hexToRgba(primaryColor, alpha)}
              >
                {b.initials}
              </SvgText>
            </G>
