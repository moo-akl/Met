  ...
  return (
    ...
    >
      ...
      <div className="text-center">
        ...
        >
          Detect who's{' '}
          <span className="text-[var(--color-accent)] drop-shadow-[0_0_10px_rgba(58,224,106,0.4)]">nearby.</span>
        </motion.h2>
      </div>
      ...
      <PhoneMockup>
        <div className="w-full h-full pt-8 px-4 flex flex-col">
          ...
          <div className="relative w-full aspect-square rounded-full border border-[var(--color-accent)]/20 mb-4 flex items-center justify-center">
            ...
            {phase >= 2 && (
              <motion.div
                className="absolute w-[7px] h-[7px] rounded-full bg-[var(--color-accent)] top-[22%] left-[28%]"
            ...
            {phase >= 3 && (
              <motion.div
                className="absolute w-[7px] h-[7px] rounded-full bg-[var(--color-accent)] bottom-[28%] right-[22%]"
                style={{ boxShadow: '0 0 8px var(--color-accent)' }}
                initial={{ scale: 0, opacity: 0 }}
                animate={{ scale: [0, 1.6, 1], opacity: [0, 1, 0.85] }}
                transition={{ duration: 0.5 }}
            ...
            {phase >= 4 && (
              <motion.div
                className="absolute w-[6px] h-[6px] rounded-full bg-[var(--color-accent)] top-[55%] left-[60%]"
                style={{ boxShadow: '0 0 6px var(--color-accent)' }}
                initial={{ scale: 0, opacity: 0 }}
                animate={{ scale: [0, 1.5, 1], opacity: [0, 1, 0.75] }}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              { label: 'Pending', val: '2', accent: true },
            ].map((s, i) => (
              <motion.div
                key={i}
              ...
              >
                <div className="text-[7px] text-[var(--color-text-secondary)] uppercase tracking-wider mb-1">{s.label}</div>
                <div className={`text-base font-bold ${s.accent ? 'text-[var(--color-accent)]' : 'text-[var(--color-text-primary)]'}`}>{s.val}</div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      >
        ...
        />
        <span className="text-[9px] tracking-[0.18em] text-[var(--color-text-secondary)] font-semibold uppercase">LIVE SCAN</span>
      </motion.div>
    </motion.div>
  );
}
